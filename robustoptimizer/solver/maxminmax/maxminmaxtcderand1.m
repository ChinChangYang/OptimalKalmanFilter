function [xbest1, xbest2, xbest3, fbest, out] = maxminmaxtcderand1(...
...
% MAXMINMAXTCDERAND1 Max-Min-Max Tracer DE/rand/1
% [xbest1, xbest2, xbest3] = MAXMINMAXTCDERAND1(fitfun, maxfunevals, lb1,
...
% maxfunevals.
% MAXMINMAXTCDERAND1(..., options1) maximizes the function with the given
% options Options1 for the 1st layer.
% MAXMINMAXTCDERAND1(..., options1, options2) maximizes the function with
% the given options Options2 for the 2nd layer.
% MAXMINMAXTCDERAND1(..., options1, options2, options3) maximizes the
% function with the given options Options3 for the 3rd layer.
% [..., fbest] = MAXMINMAXTCDERAND1(...) returns the function value of the
...
defaultOptions1.InnerSolver = 'minmaxtcderand1bin';
...
defaultOptions2.InnerSolver = 'derand1bin';
	...
	for i = 1 : NP1
		...
		for retry_within_bounds = 1 : NP1
			...
			if all(V(:, i) >= lb1) && all(V(:, i) <= ub1)
				...
				for j = 1 : NP2
					V3(:, :, j, i) = innerState{r1}.innerState{j}.X + Fi .* ...
						(innerState{r2}.innerState{j}.X - ...
						innerState{r3}.innerState{j}.X);
				end
